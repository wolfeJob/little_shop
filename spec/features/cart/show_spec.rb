...
describe 'Cart Show Page' do
 describe 'When I view my cart' do
   before :each do
     ...
     @pie = @merchant_1.items.create(name: 'Pie', description: "Yummy Pie!", price: 15, image: 'https://upload.wikimedia.org/wikipedia/commons/thumb/9/91/Tarte_aux_poires_2a.jpg/1920px-Tarte_aux_poires_2a.jpg(488 kB)
https://upload.wikimedia.org/wikipedia/commons/thumb/9/91/Tarte_aux_poires_2a.jpg/1920px-Tarte_aux_poires_2a.jpg
', inventory: 2 )
...
', inventory: 10 )
   ...
   it 'it shows all items and each item displays the name, image, merchant, price, quantity, subtotal, and grand total' do
     ...
     within "#item-#{@pie.id}" do
       ...
       expect(page).to have_content("Subtotal: #{number_to_currency(@pie.price)}")
     end
     ...
     # end
     expect(page).to have_content("Total: #{number_to_currency((@pie.price))}")
   end
